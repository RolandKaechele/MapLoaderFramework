...
- **LoadScreenManager integration** — `MapLoaderLoadScreenBridge` in LoadScreenManager auto-shows the load screen on `OnChapterChanged` and hides it on `OnMapLoaded` (activated via `LOADSCREENMANAGER_MLF` in the LoadScreenManager module)
